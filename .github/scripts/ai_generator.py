...
def generate_code(prompt: str) -> str:
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": """You are an expert code generator. Generate clean, well-documented, 
                production-ready code. Include comments and type hints. Return ONLY the code, no explanations."""
            },
            {
                "role": "user",
...
def create_branch_name(prompt: str) -> str:
    ...
    # Use GPT to create a good branch name
    response = client.chat.completions.create(
        model="gpt-3.5-turbo",
        messages=[
            {
                "role": "system",
                "content": "Generate a short, valid git branch name (lowercase, hyphens, max 50 chars). Return ONLY the branch name."
            },
            {
                "role": "user",
                "content": f"Create a branch name for: {prompt}"
            }
        ],
        max_tokens=20
    )
    ...
    branch_name = branch_name.strip("-")[:50]
...
def detect_language(prompt: str) -> str:
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "Detect the programming language from the prompt. Return ONLY the file extension (py, js, ts, java, etc) or 'txt' if unclear."
            },
            {
                "role": "user",
                "content": prompt
    ...
    return ext if ext in ["py", "js", "ts", "java", "cpp", "go", "rs", "rb"] else "txt"
...
def main():
    ...
    meta_file = Path(".ai_generation_metadata.json")
